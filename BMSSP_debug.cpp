...
private:
    ...
    pair<double, set<int>> base_case(double B, const set<int>& S) {
        ...
        if ((int)U0.size() <= k) {
            cout << "    Base case: retornando todo, |U0|=" << U0.size() << endl;
            return {B, U0};
        } else {
            double B_prime = -INF;
            for (int v : U0) {
            ...
            cout << "    Base case: retornando parcial, B'=" << B_prime << ", |U|=" << U.size() << endl;
            return {B_prime, U};
        }
    }
...
public:
    ...
    pair<double, set<int>> bmssp(int level, double B, const set<int>& S) {
        cout << "BMSSP llamado: level=" << level << ", B=" << B << ", |S|=" << S.size() << endl;

        // Caso base
        if (level == 0) {
            auto result = base_case(B, S);
            cout << "  Base case retorna: B'=" << result.first << ", |U|=" << result.second.size() << endl;
            return result;
        }

        // Encontrar pivotes
        auto [P, W] = find_pivots(B, S);
        cout << "  FindPivots: |P|=" << P.size() << ", |W|=" << W.size() << endl;
        ...
        while ((int)U.size() < threshold && !D.is_empty() && i < max_iterations) {
            i++;
            cout << "    Iteracion " << i << ": |U|=" << U.size() << endl;

            // Pull
            auto [S_i_vec, B_i] = D.pull();
            set<int> S_i(S_i_vec.begin(), S_i_vec.end());

            cout << "      Pull: |S_i|=" << S_i.size() << ", B_i=" << B_i << endl;

            if (S_i.empty()) {
                cout << "      S_i vacio, saliendo" << endl;
                break;
            }

            // Llamada recursiva
            cout << "      Llamando recursion..." << endl;
            auto [B_prime_i, U_i] = bmssp(level - 1, B_i, S_i);
            cout << "      Recursion retorno: B'_i=" << B_prime_i << ", |U_i|=" << U_i.size() << endl;
            U.insert(U_i.begin(), U_i.end());

            // Relajar aristas y actualizar D
            vector<pair<int, double>> K;
            cout << "      Relajando aristas de " << U_i.size() << " vertices..." << endl;
            int relaxed_count = 0;
            for (int u : U_i) {
                for (const Edge& e : graph.adj[u]) {
                    if (relax_edge(u, e.to, e.weight)) {
            ...
            cout << "      Aristas relajadas: " << relaxed_count << ", |K|=" << K.size() << endl;
        ...
        cout << "  BMSSP retorna: B'=" << B_prime_prev << ", |U|=" << U.size() << endl;
        return {B_prime_prev, U};
    }

    map<int, double> solve_sssp(int source) {
        cout << "Iniciando SSSP desde vertice " << source << endl;
        cout << "Parametros: n=" << n << ", k=" << k << ", t=" << t << endl;

        db[source] = 0.0;

        // Calcular nivel máximo (limitado para grafos pequeños)
        int max_level = 1; // Simplificado para debugging
        if (n > 10) {
        ...
        cout << "BMSSP completado. Vertices alcanzados: " << U.size() << endl;
